...
export const CreateEditSingle = ({ onSave, prefilledReminderData }) => {
  ...
  useEffect(() => {
    ...
    if (hasPrefilledData) {
      ...
      setReminderDate(
        format(new Date(prefilledReminderData.notification_date), "MM/dd/yyyy")
      );
      setFormData({
        subject: prefilledReminderData.subject,
        description: prefilledReminderData.description,
        frequency_type: prefilledReminderData.frequency_type,
        is_anchored: prefilledReminderData?.is_anchored ? true : false,
        anchor_number: prefilledReminderData?.anchor_number ?? 0,
      });
    }
  ...
  const onFrequencyChange = ({
    ...
    if (!onUserSelectDate && !hasPrefilledData) {
      ...
      if (reminderData.notification_date) {
        notificationDateObject = new Date(reminderData.notification_date);

        setReminderDate(format(notificationDateObject, "MM/dd/yyyy"));
        currentReminderDate = format(notificationDateObject, "MM/dd/yyyy");
  ...
  return (
    <>
      ...
      <Switch
        value={formData.is_anchored ?? false}
        onValueChange={(value) => {
          setFormData({
            ...formData,
            is_anchored: value,
      ...
      <TextInput
        label="Date"
        value={formData.anchor_number}
        onChangeText={(text) => {
          onChange("anchor_number", text);
        }}
        testID="anchor_number"
      />
      <Button mode="contained" onPress={handleSubmit} testID="submit_form">
        Submit
      </Button>
    </>
  );
