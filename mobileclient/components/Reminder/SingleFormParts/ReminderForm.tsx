...
import { BaseOptions } from "./FrequencyOptions";
    ...
    <>
      ...
      >
        {errors.map((currentError: string) => {
          return <Text style={{ color: "white" }}>*{currentError}</Text>;
        })}
      </Snackbar>
    </>
  );
...
const ReminderForm = ({ onSave, onCancel, serverErrors, prefillData }) => {
  ...
  useEffect(() => {
    if (prefillData) {
      ...
      setFormData((v) => {
        return {
          ...v,
          is_anchored: prefillData?.is_anchored ?? false,
          anchor_number: prefillData?.anchor_number ?? 0,
        };
      });
    }
  }, [prefillData]);
  ...
  const onBaseOptionChange = (p) => {
    ...
    setNextDate(calculateNextDate(currentDate, p.frequency, p.frequencyType));
  };
  const onUserSelectDate = (currentDateObject: Date) => {
    setCurrentDate(new Date(currentDateObject));
    setNextDate(calculateNextDate(currentDateObject, frequency, frequencyType));
  };
  const validateAndSave = (): bool => {
    ...
    if (frequencyType === "m") {
      savePayload = {
        ...savePayload,
        is_anchored: formData.is_anchored,
        anchor_number: formData.anchor_number,
      };
    }

    onSave ? onSave(savePayload) : "";
  };
        ...
        {frequencyType === "m" && (
          ...
          >
            <Switch
              onValueChange={(value) => {
                setFormData((v) => {
                  return { ...v, is_anchored: value };
                });
              }}
              value={formData.is_anchored}
            />
            {formData.is_anchored && (
              <TextInput
                label="Number"
                mode="outlined"
                value={
                  formData?.anchor_number
                    ? formData.anchor_number.toString()
                    : ""
                }
                style={{ width: "30%", padding: 1, marginLeft: "2%" }}
                onChangeText={(text) => {
                  setFormData((v) => {
                    return { ...v, anchor_number: text };
                  });
                }}
              />
            )}
