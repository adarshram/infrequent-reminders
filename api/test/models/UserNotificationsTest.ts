...
import { differenceInCalendarDays, addDays, getUnixTime, addMonths, setDate } from 'date-fns';
...
describe('save new reminder set', () => {
  ...
  it('create two types of notification', async () => {
    const user_id = '123';
    await deleteNotificationsForUser(user_id);
    let notificationParameters = {
      user_id: user_id,
      subject: '12312312',
      description: 'scdsacsac',
    ...
    let monthAnchor = await createNotificationsForUser(notificationParameters);
    await completeNotification(monthAnchor.id, monthAnchor.user_id);
    let completedMonthAnchor = await getNotificationById(monthAnchor.id, monthAnchor.user_id);

    let nextMonth = addMonths(notificationParameters.notification_date, 1);
    let nextMonthDate = setDate(nextMonth, notificationParameters.anchor_number);
    expect(format(completedMonthAnchor.notification_date, 'yyyy-MM-dd')).to.be.equal(
      format(nextMonthDate, 'yyyy-MM-dd'),
    );

    await deleteNotification(monthAnchor.id);
    ...
    monthAnchor = await createNotificationsForUser(notificationParameters);
    await completeNotification(monthAnchor.id, monthAnchor.user_id);
    completedMonthAnchor = await getNotificationById(monthAnchor.id, monthAnchor.user_id);
    nextMonth = addMonths(notificationParameters.notification_date, 1);
    expect(format(completedMonthAnchor.notification_date, 'yyyy-MM-dd')).to.be.equal(
